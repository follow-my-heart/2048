...
(function () {
    ...
    _$.fn = _$.prototype = {
        ...
        css: function css(attr, _css) {
            switch (arguments.length) {
                case 1:
                    //首先判断是否是json数据，是的话那就依次设置（设置）
                    if ((typeof attr === 'undefined' ? 'undefined' : _typeof(attr)) == 'object' && Object.prototype.toString.call(attr).toLowerCase() == "[object object]" && !attr.length) {
                        //遍历自己
                ...
                case 2:
                    if (_css) {
                        //如果有两个参数那必然是遍历设置
                        if (this.length == 1) {
                            this[0].style[attr] = _css;
                            return this;
                        }
                        this.each(function (i, item) {
                            this.style[attr] = _css;
                        });
                    }
                    return this;
                default:
...
(function () {
    ...
    //这里将会未来常量的声明区域
    var BLOCK_SIZE = 88;
    var getVector = function getVector(direction) {
        return [{
            x: -1,
            y: 0
        }, {
    ...
    var getDirectionList = function getDirectionList(direction, size) {
        ...
        this.each(function (item) {
            if (item) {
                ...
                for (var i in neighbor) {
                    if (neighbor[i] && this.withinBound(neighbor[i]) && item.value == neighbor[i].value) {
                        flag = true;
                        return true;
                    }
                }
            ...
            directionList.y.forEach(function (y) {
                ...
                if (self.availableCell(position)) {
                    ...
                    if (self.withinBound(cellResult.next)) {
                        if (self.getCellContent(cellResult.next) && self.getCellContent(position) && !self.getCellContent(cellResult.next).mergedFrom && self.getCellContent(cellResult.next).value == self.getCellContent(position).value) {
                ...
                if (this.cells[i][j]) {
                    ...
                    if (this.cells[i][j].previousPosition) {
                        if (this.cells[i][j].mergedFrom) {
                            ...
                            _from = $.div().addClass('tile-' + value).addClass('tile-cell').css({
                                'top': BLOCK_SIZE * prey + 16 * prey + 'px',
                                'left': BLOCK_SIZE * prex + 16 * prex + 'px'
                            });
                            _tile = $.div().addClass('tile-' + value).addClass('tile-cell').css({
                            ...
                            _tile = $.div().addClass('tile-' + this.cells[i][j].value).addClass('tile-cell').css({
                                'top': BLOCK_SIZE * prey + 16 * prey + 'px',
                                'left': BLOCK_SIZE * prex + 16 * prex + 'px'
                            });
                            _font = document.createElement('font');
                        ...
                        _tile = $.div().addClass('tile-' + this.cells[i][j].value).addClass('tile-cell').addClass('tile-new').css({
    ...
    function GameManage(contentId, size) {
        ...
        this.controlBox.append('<p class="control">' + '<button>再次尝试</button>' + '</p>');
